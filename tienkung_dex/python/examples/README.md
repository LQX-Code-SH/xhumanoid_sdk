...
- **`--backend sim`**：ros_gz 仿真（推荐与 20/09 组合验证）。
  - **sim 适用集（2026-09-06 全量 22 例回归实测）**：通过 = 03/05/10/11/16/18/19/20（退出码 0）；预期差异 = 04（gz 无 IMU 传感器）/07（gz 无雷达）/09（waist 相机 gz 模型无，head/wrist 三组正常）/12（无 AIUI 事件源）/13（无录音源）/14（无触觉通道，`on_touch` 静默；`get_status` 正常）；不支持 = 01/02/06/08/15（门面属性 None → `AttributeError`，见主 README 限制 #14）/17（`BackendUnavailableError`）/21（walk 无 sim）/22（仅 mock，exit 2）。运动/手类 18/19/20/16 到位与回零全部达标。
